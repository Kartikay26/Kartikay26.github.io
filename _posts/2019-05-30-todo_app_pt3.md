...
But as we don't really need it here, we'll not add it. We may need something like this if there's some online competition and we want to refresh the scoreboard everytime, say. Right now let's only refresh the data if something changes.
